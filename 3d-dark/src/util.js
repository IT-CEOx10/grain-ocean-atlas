...
(function (global) {
  ...
  /* ----------------------- стенд: сенсорная панель -----------------------
     Приложение работает на тач-панели без клавиатуры и мыши. Посетитель
     не должен случайно вызвать меню браузера, выделить текст, утащить
     картинку или увеличить страницу щипком: сцена рассчитана на весь экран.
     Свои жесты разделов (вращение глобуса, перетаскивание карты) идут
     через pointer-события на холстах и этим запретам не мешают.
     Отключить для отладки: ?kiosk=0 в адресе. */
  (function kiosk() {
    if (/[?&]kiosk=0\b/.test(global.location.search)) return;
    ...
    ['gesturestart', 'gesturechange', 'gestureend'].forEach(function (n) {
      doc.addEventListener(n, stop);
    });
    // масштаб страницы: Ctrl + колесо и Ctrl + «плюс/минус/ноль»
    doc.addEventListener('wheel', function (e) { if (e.ctrlKey) e.preventDefault(); },
      { passive: false });
  ...
  /* Служебные пометки («[VIDEO]», «тексты предоставит заказчик» и т. п.)
     нужны при работе над стендом, но не посетителю. По умолчанию скрыты,
     показать: ?notes=1 в адресе. Элементы помечаются классом is-note
     или перечислены ниже. */
  (function notes() {
    if (/[?&]notes=1\b/.test(global.location.search)) return;
    var st = global.document.createElement('style');
    st.textContent = '.is-note,.orb-tag,.news-note,.mn-pres-todo{display:none !important}';
