...
for pair in "$@"; do
  n="${pair%%=*}"; q="${pair#*=}"
  # запрос вида «screen=hub» — экран презентации; «page.html?…» — любая другая страница стенда
  case "$q" in
    *.html*) url="http://localhost:8765/$q&idle=0&cb=$RANDOM" ;;
    *)       url="http://localhost:8765/story.html?$q&idle=0&cb=$RANDOM" ;;
  esac
  prof="$OUT/.ch-$n"; rm -rf "$prof" "$OUT/$n.png"
